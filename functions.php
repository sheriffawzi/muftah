...
if ( ! function_exists( 'cb_scripts_and_styles' ) ) {
    function cb_scripts_and_styles() {
      if (!is_admin()) {
        ...
        if ( is_rtl() ) {
            wp_register_style( 'cb-main-stylesheet',  get_template_directory_uri() . '/library/css/style-rtl.css', array(), '2.2', 'all' );
        } else {
            $cb_responsive_style = ot_get_option('cb_responsive_onoff', 'on');
            if ( $cb_responsive_style == 'on' ) {
                wp_register_style( 'cb-main-stylesheet',  get_template_directory_uri() . '/library/css/style.css', array(), '2.2', 'all' );
            } else {
                wp_register_style( 'cb-main-stylesheet',  get_template_directory_uri() . '/library/css/style-unresponsive.css', array(), '2.2', 'all' );
            }
        }
        wp_enqueue_style('cb-main-stylesheet'); // enqueue it
        // Register fonts
        $cb_font = cb_fonts();
        wp_register_style( 'cb-font-stylesheet',  $cb_font[0], array(), '2.2', 'all' );
        ...
        // ie-only stylesheet
        wp_register_style( 'cb-ie-only',  get_template_directory_uri(). '/library/css/ie.css', array(), '2.2' );
        wp_enqueue_style('cb-ie-only'); // enqueue it
        if ( class_exists('Woocommerce') ) {
            wp_register_style( 'cb-woocommerce-stylesheet',  get_template_directory_uri() . '/woocommerce/css/woocommerce.css', array(), '2.2', 'all' );
        ...
        // Load scripts
        wp_register_script( 'cb-js',  get_template_directory_uri() . '/library/js/cb-scripts.js', array( 'jquery' ), '2.0' , true);
...
if ( ! function_exists( 'cb_post_admin_scripts_and_styles' ) ) {
    function cb_post_admin_scripts_and_styles($hook) {
        ...
        if( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'profile.php' || $hook == 'appearance_page_ot-theme-options' || $hook == 'user-edit.php' || $hook == 'edit-tags.php' ) {
            wp_register_style( 'cb-admin-css',  get_template_directory_uri(). '/library/css/admin.css', array(), '2.2' );
            wp_enqueue_style('cb-admin-css'); // enqueue it
            wp_register_script( 'admin-js',  get_template_directory_uri() . '/library/js/cb-admin.js', array(), '2.2', true);
...
if ( ! function_exists( 'cb_register_sidebars' ) ) {
    function cb_register_sidebars() {
        ...
        register_sidebar(
            array(
                'name' => 'Muftah Multi-Widgets Area',
                'id' => 'cb_multi_widgets',
                'description' => '1- Drag multiple widgets here 2- Drag the "Muftah Multi-Widget Widget" to the sidebar where you want to show the multi-widgets.',
...
/*
function muftah_excerpt_length( $length ) {
	return 30;
}
add_filter( 'excerpt_length', ' muftah_excerpt_length', 999 );

function muftah_excerpt_more( $more ) {
	//return ' <a class="read-more" href="'. get_permalink( get_the_ID() ) . '">Read More</a>';
	return '...';
}
add_filter( 'excerpt_more', 'muftah_excerpt_more' );
*/
?>