   ...
   $('.cb-enabled-review > div:gt(3):odd:lt(6)').each(function() {
    ...
    var cbReviewCheckbox = $('#cb_review_checkbox'),
      ...
      cbSelectedAd.each(function () {
         ...
         var cbCurrentSection = $(this).closest("[id^=cb_cb]").attr('id'),
      ...
      cbSectionA.add(cbSectionB).add(cbSectionC).add(cbSectionD).on('click', '.option-tree-ui-radio-image', function() {
          ...
          var cbCurrentBlock = $(this).closest('.option-tree-setting-body'),
              cbCurrentSection = $(this).closest("[id^=cb_cb_section_]").attr('id'),
              cbCurrentSectionID = cbCurrentSection.substr(cbCurrentSection.length - 1),
        ...
        if ( ( cbCatGridSliderValue === 'Off' ) || ( cbCatGridSliderValue === 'Full-width Slider' ) || ( cbCatGridSliderValue === 'Slider' ) ) {
          cbCatOffset.hide();
        }

        cbCatGridSlider.change(function() {

          if ( ( this.value === 'Off' ) || ( this.value === 'full-slider' ) || ( this.value === 'slider' ) ) {
            cbCatOffset.slideUp();
          } else {
