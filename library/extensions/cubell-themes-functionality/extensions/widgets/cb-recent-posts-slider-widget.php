...
if ( ! class_exists( 'CB_widget_recent_posts_slider' ) ) {
    class CB_widget_recent_posts_slider extends WP_Widget {
    	...
    	function widget($args, $instance) {
    		...
    		if ( empty( $instance['number'] ) || ! $number = absint( $instance['number'] ) )$number = 6;
            
            if ($category != 'cb-all') { $cb_cat_qry = $category;} else {$cb_cat_qry = NULL;}
    
    		$cb_qry = new WP_Query( apply_filters( 'widget_posts_args', array( 'posts_per_page' => $number, 'no_found_rows' => true, 'category_name' => $cb_cat_qry, 'post_status' => 'publish', 'ignore_sticky_posts' => true ) ) );
