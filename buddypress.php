<?php
        ...
        if ( ( ( $cb_bp_current_component == 'activity' ) || ( $cb_bp_current_component == 'profile' ) ) && ( bp_is_directory() == false ) && ( $cb_bp_current_action != NULL ) ) {
            $cb_title_prefix = '<span>'. __( 'Member', 'buddypress' ) . ' <i class="icon-long-arrow-right"></i></span> ';
        }

        if ( ( $cb_bp_current_component == 'groups' )   && ( $cb_bp_current_action != NULL ) ) {
            $cb_title_prefix = '<span>'. __( 'Group', 'buddypress' ) . ' <i class="icon-long-arrow-right"></i></span> ';
        }
        ...
        if ( ( $cb_bp_current_component == 'settings' ) ) {
            $cb_title_prefix = '<span>'. __( 'Settings', 'buddypress' ) . ' <i class="icon-long-arrow-right"></i></span> ';
        }

        if ( ( $cb_bp_current_component == 'forums' ) ) {
            $cb_title_prefix = '<span>'. __( 'Forums', 'buddypress' ) . ' <i class="icon-long-arrow-right"></i></span> ';
        }
        ...
        if ( ( $cb_bp_current_component == 'groups' ) && ( $cb_bp_current_action == NULL ) ) {
            $cb_title = '<span>'. __( 'Groups', 'buddypress' ) . ' ' . '<a class="cb-group-create cb-tip-bot" title="' . __( 'Create a Group', 'buddypress' ) . '" href="' . trailingslashit( bp_get_root_domain() . '/' . bp_get_groups_root_slug() . '/create' ) . '"><i class="icon-plus"></i></a>';
            $cb_group_creator = NULL;
        }

        if ( ( $cb_bp_current_component == 'groups' ) && ( $cb_bp_current_action == 'create' ) ) {
            $cb_title_prefix = '<span>'. __( 'Groups', 'buddypress' ) . ' <i class="icon-long-arrow-right"></i></span> ';
